...
// Requires window.QRCode, window.jspdf.jsPDF, window.JSZip

import {
  getPlan, canRender, incrementRenderCount,
  shouldWatermark, getDelay, isPro
} from './licensing.js';
...
function showProToast(msg='This feature is available in Pro.'){
  ...
  const span=$('#proToastMsg'); const icon=t.querySelector('.toast-icon'); const strong=t.querySelector('.toast-text strong');
  if(icon) icon.style.display=''; if(strong){ strong.textContent='Pro feature'; strong.style.display=''; }
  if(span) span.textContent=' '+msg+' ';
  t.classList.remove('hidden'); clearTimeout(window.__proToastTimer); window.__proToastTimer=setTimeout(hideProToast,2200);
}
function showInfoToast(msg=''){ const t=$('#proToast'); if(!t) return;
  const span=$('#proToastMsg'), icon=t.querySelector('.toast-icon'), strong=t.querySelector('.toast-text strong');
  if(icon) icon.style.display='none'; if(strong) strong.style.display='none';
  if(span) span.textContent=' '+msg+' ';
  t.classList.remove('hidden'); clearTimeout(window.__proToastTimer); window.__proToastTimer=setTimeout(hideProToast,2400);
}
...
$('#startNow').addEventListener('click', ()=>{ tabBtns.find(b=>b.dataset.tab==='builder').click(); });

/* Mini-tool cards */
$('#toolEsim').addEventListener('click', ()=>{ tabBtns.find(b=>b.dataset.tab==='builder').click(); $('input[name="mode"][value="esim"]').checked=true; mode='esim'; renderCustomBuilder(); });
$('#toolWifi').addEventListener('click', ()=>{ tabBtns.find(b=>b.dataset.tab==='builder').click(); $('input[name="mode"][value="wifi"]').checked=true; mode='wifi'; renderCustomBuilder(); });
$('#toolSheets').addEventListener('click', ()=>{ tabBtns.find(b=>b.dataset.tab==='sheets').click(); if(!isPro()) showProToast('Label / Sheet Maker is Pro.'); });

/* Plan chrome: set FREE/PRO badge, disable locked buttons */
function applyPlanChrome(){
  const plan=getPlan(); $('#planBadge').textContent=plan.name.toUpperCase();
  const locked = !isPro();
  ['runBatch','runBatchPDF','makeSheetFromCurrent','previewPDF'].forEach(id=>{
    const el = $('#'+id); if(!el) return;
    if (locked){ el.classList.add('is-locked'); el.setAttribute('aria-disabled','true'); el.addEventListener('click',stopLock, { once:true }); }
    else{ el.classList.remove('is-locked'); el.removeAttribute('aria-disabled'); }
  });
  function stopLock(e){ e.preventDefault(); showProToast('This action requires Pro.'); }
}

/* State / elements */
let presets=[]; let selectedPresetId=null; let mode='esim';
let currentQRData=''; let currentPNGBlob=null; let currentSVGText=null;
...
function renderPresets(){
  const top=$('#topPresets'), other=$('#otherPresets'); top.innerHTML=''; other.innerHTML='';
...
function renderCustomBuilder(){
  ...
  if (mode==='esim'){
    ...
    cp.innerHTML=`
      ...
      <div style="grid-column:1/-1"><label for="raw">Raw string (editable)</label><textarea id="raw" rows="3" class="mono" placeholder="WIFI:T:..."></textarea></div>
      <div class="row" style="grid-column:1/-1"><button id="btnGen" class="btn btn-primary">Generate QR</button><button id="btnClear" class="btn btn-secondary" type="button">Clear</button></div>`;
  }

  const sdmp=$('#sdmp'), match=$('#match'), act=$('#act'), conf=$('#conf');
  const ssid=$('#ssid'), sec=$('#sec'), wpass=$('#wpass'), hidden=$('#hidden');
  const raw=$('#raw'); const btnGen=$('#btnGen'); const btnClear=$('#btnClear');

  function syncRaw(){
    if(mode==='esim'){
      raw.value = buildLPA({ presetId:p.id, sdmp:sdmp?.value||'', match:match?.value||'', act:act?.value||'', conf:conf?.value||'' });
    } else {
      raw.value = buildWiFi({ ssid:ssid?.value||'', security:(sec?.value||'WPA'), pass:(sec?.value==='nopass'?'':(wpass?.value||'')), hidden:(hidden?.value==='true') });
    }
  }
  ...
  btnClear?.addEventListener('click', ()=>{
    [sdmp, match, act, conf, ssid, sec, wpass, hidden].forEach(inp=>{ if(!inp) return; if(inp.tagName==='SELECT') inp.selectedIndex=0; else inp.value=''; });
    syncRaw(); resultSection.classList.add('hidden'); downloadPNG.removeAttribute('href'); downloadSVG.removeAttribute('href'); currentQRData=''; currentPNGBlob=null; currentSVGText=null; updatePrintIndicator(null);
  });
...
async function handleGenerate(text){
  ...
  try{
    ...
    if (shouldWatermark()){
      currentSVGText = currentSVGText.replace('</svg>', `<text x="50%" y="98%" dominant-baseline="middle" text-anchor="middle" font-size="12" fill="#555" font-family="sans-serif">Generated with Connectivity QR (Free)</text></svg>`);
    }
    const svgBlob = new Blob([currentSVGText], {type:'image/svg+xml'}); downloadSVG.href = URL.createObjectURL(svgBlob);

    currentQRData=text; qrStringEl.textContent=text; resultSection.classList.remove('hidden');
...
copyBtn.addEventListener('click', async ()=>{ const text=qrStringEl.textContent||''; if(!text) return; try{ await navigator.clipboard.writeText(text); showInfoToast('String copied.'); }catch(e){ console.error(e); } });
...
function renderHistory(){
  const h=loadHistory(); const wrap=$('#historyList'); wrap.innerHTML=''; if(!h.length){ wrap.innerHTML='<div class="section-tip">No items yet.</div>'; return; }
  h.slice(-10).reverse().forEach(it=>{ const d=new Date(it.when||Date.now()); const item=document.createElement('div'); item.className='history-item';
    ...
    item.addEventListener('click', ()=>{ tabBtns.find(b=>b.dataset.tab==='builder').click(); handleGenerate(it.content); });
...
/* Label / Sheets (from current QR) */
$('#makeSheetFromCurrent').addEventListener('click', async ()=>{
  if(!isPro()) return showProToast('Label / Sheet Maker is Pro.');
...
$('#previewPDF').addEventListener('click', async ()=>{
  ...
  const blob=await buildA4PDF({ tiles, logo }); if(!blob) return;
...
async function readLogoFile(){ const f=$('#logoFile').files[0]; if(!f) return null; const buf=await f.arrayBuffer(); const blob=new Blob([buf],{type:f.type||'image/png'}); return URL.createObjectURL(blob); }
...
/* Validate / Decode */
$('#btnValidate').addEventListener('click', ()=>{ const s=$('#valInput').value.trim(); doValidateString(s); });
$('#decodeFile').addEventListener('change', async (e)=>{
  const f=e.target.files[0]; if(!f) return;
  try{
    const imgURL=URL.createObjectURL(f);
    if('BarcodeDetector' in window){
      const det=new window.BarcodeDetector({ formats:['qr_code'] }); const img=new Image();
      ...
      if(results && results[0] && results[0].rawValue){ doValidateString(results[0].rawValue); return; }
    }
    showInfoToast('QR decoding not supported in this browser.');
  }catch(err){ console.error(err); showInfoToast('Decode failed.'); }
});
function doValidateString(s){
  const badge=$('#valStatus'), details=$('#valDetails'); details.textContent='';
...
/* Mode switch */
$$('input[name="mode"]').forEach(r=> r.addEventListener('change',(e)=>{ mode=e.target.value; renderCustomBuilder(); }));

/* Init */
async function init(){
  applyPlanChrome();
  await loadPresets(); renderPresets(); renderCustomBuilder(); renderHistory(); tset();
